<template>
  <table>
    ...
    <tbody>
    <tr v-for="weatherForSelectedDay in weatherForSelectedDays" :key="weatherForSelectedDay.expectedTemperature">
      <td>{{ weatherForSelectedDay.time }}</td>
      <td><img v-bind:src="`http://openweathermap.org/img/wn/${weatherForSelectedDay.icon}.png` "/></td>
      <td>{{ weatherForSelectedDay.expectedTemperature }}</td>
    </tr>
    </tbody>
  </table>
</template>
      ...
      axios.get('https://api.openweathermap.org/data/2.5/onecall', {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          if (currentDate.getHours() < 10) {
            currentTime = '0' + currentDate.getHours() + ':' + currentDate.getMinutes() + currentDate.getSeconds()
          } else currentTime = currentDate.getHours() + ':' + currentDate.getMinutes() + currentDate.getSeconds()
